...
//        values.put(DBHelper.JOB__ID, new_job.getId());
    ...
    public Job[] getJobsByTaskId(long taskId) {
        Job[] jobs = null;
        Cursor cursor = database.query(DBHelper.JOB_TABLE,
                allColumns, DBHelper.JOB_MAIN_TASK_JOB + "=? AND " + DBHelper.JOB_STATE + " =?", new String[]{String.valueOf(taskId), "0"}, null, null, null);
        cursor.moveToFirst();
    ...
    public Job[] getJobsByTaskIdWithAdditionalData(long taskId) {
        Job[] jobs = null;
        String sql = String.format("SELECT %s.*, task.importance, task.author_code FROM %s, %s, %s WHERE %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=0 AND %s.%s=?;", 
                ...
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE,
    ...
    public List<Job> getImportantJobByPerformerCode(String code) {
        ...
        String sql = String.format("SELECT %s.*, task.importance, task.author_code FROM %s, %s, %s WHERE %s.%s='Высокая' AND %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=0 AND %s.%s=?;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
...
//            job.setSubtaskCount(task_ds.getTaskById(job.getMainTaskJob()).getSubtaskCount());
    ...
    public List<Job> getFavoriteJobByPerformerCode(String code) {
        ...
        String sql = String.format("SELECT %s.*, task.importance, task.author_code FROM %s, %s, %s WHERE %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND %s.%s=1 AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID,
...
//            job.setSubtaskCount(task_ds.getTaskById(job.getMainTaskJob()).getSubtaskCount());
    ...
    public List<Job> getImportantFavoriteJobByPerformerCode(String code) {
        ...
        String sql = String.format("SELECT %s.*, task.importance, task.author_code FROM %s, %s, %s WHERE (%s.%s='Высокая' OR %s.%s=1) AND %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                ...
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID,
                DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.JOB_TABLE, DBHelper.JOB_STATE);
//        Log.v("getImportanceJobByPerformerCode()", "sql " + sql);
        Cursor cursor = database.rawQuery(sql, new String[]{code});
        cursor.moveToFirst();
        Job job = null;
...
//            job.setSubtaskCount(task_ds.getTaskById(job.getMainTaskJob()).getSubtaskCount());
    ...
    public List<Job> getJobByPerformerCode(String code, String filter) {
        ...
        String sql = String.format("SELECT %s.*, task.importance, task.author_code FROM %s, %s, %s WHERE %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=0 AND %s.%s=? %s;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID,
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE,
...
//        Log.v("getJobByPerformerCode()", "sql " + sql);
    ...
    public int[] getCountOfImportantJobByPerformerCode(String code) {
        int[] count = {0, 0, 0};
        // green count
        String sql = String.format("SELECT count(%s._id) FROM %s, %s, %s WHERE %s.%s='Высокая' AND %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.TASK_TABLE, DBHelper.TASK_IMPORTANCE, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
        ...
        cursor.close();
        // yellow count
        sql = String.format("SELECT count(%s._id) FROM %s, %s, %s WHERE %s.%s='Высокая' AND %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND job.final_date = date() AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.TASK_TABLE, DBHelper.TASK_IMPORTANCE, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER,
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE);
        ...
        cursor.close();
        // red count
        sql = String.format("SELECT count(%s._id) FROM %s, %s, %s WHERE %s.%s='Высокая' AND %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND job.final_date < date() AND job.final_date <> '1899-12-30 00:00:00' AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.TASK_TABLE, DBHelper.TASK_IMPORTANCE, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER,
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE);
    ...
    public int[] getCountOfJobByPerformerCode(String code) {
        int[] count = {0, 0, 0};
        String sql = String.format("SELECT count(%s._id) FROM %s, %s, %s WHERE %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER,
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE);
        ...
        sql = String.format("SELECT count(%s._id) FROM %s, %s, %s WHERE %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND job.final_date = date() AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER,
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE);
        ...
        sql = String.format("SELECT count(%s._id) FROM %s, %s, %s WHERE %s.%s=%s.%s AND %s.%s=%s.%s AND %s.%s=? AND job.final_date < date() AND job.final_date <> '1899-12-30 00:00:00' AND %s.%s=0;", 
                DBHelper.JOB_TABLE, DBHelper.JOB_TABLE, DBHelper.TASK_TABLE, DBHelper.RABOTNIC_TABLE,
                DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER, DBHelper.RABOTNIC_TABLE, DBHelper.RABOTNIC_CODE,
                DBHelper.JOB_TABLE, DBHelper.JOB_MAIN_TASK_JOB, DBHelper.TASK_TABLE, DBHelper.TASK_ID, DBHelper.JOB_TABLE, DBHelper.JOB_PERFORMER,
                DBHelper.JOB_TABLE, DBHelper.JOB_STATE);
